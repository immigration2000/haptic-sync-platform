...
✅ **2026-08-27 실기기 확인 (전송 계층)** — 영상을 화면 밖으로 스크롤한 상태로 약 76초:
엔진 발생 수와 실제 전송 수가 한 줄도 어긋나지 않았고(2초당 6~19개), `lastError` 없음,
연결 유지, **기기도 실제로 움직였다.**
⚠ 이건 **`await` 없는 쓰기가 유실 없이 동작한다**는 증거지, 스크롤 증상의 원인을 밝힌 것이 아니다(§7 항목 참조 — 실패를 재현한 적이 없다).
...
- [x] **전송 계층(mosa식) 실기기 검증** — 2026-08-27. 76초, 유실·오류 0, 기기 정상 동작. §6-13
- [ ] ⚠ **스크롤 멈춤 증상의 원인은 여전히 미확정** — 지금은 정상이지만 **실패를 재현한 적이 없다.**
      재발 시 연결 상태(`연결=false`에서도 엔진이 명령을 계속 만들어 버린다)부터 확인할 것
